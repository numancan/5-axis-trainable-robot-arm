...
// Contains the recorded servo degrees
// recording max 30 point
byte degreesLog[30][6];

byte logCount=0;
byte currentDegrees[6]{90,90,90,90,90,90};

// Contains degrees to run
byte degreesToRun[30][6];
byte runCount=0;
...
void loop() {
  if (Serial.available()) {
    ...
    // Sample data "WS,degree"
    // Used two servo for shoulder
    else if (command=="WS") {
        byte degree=(Serial.readStringUntil(',')).toInt();
...
void Playback() {
    ...
    for(int i=0; i<runCount; i++) {
        for(int j=0; j<6; j++) {
            ...
            if(j==5) PlayContinuous(degreesToRun[i][j]);   
